...
app.post('/read', (req, res) => {
    const { fileName, sheetName: requestedSheetName } = req.body;
    if (!fileName) {
    ...
    try {
        ...
        // 2. '__EMPTY'로 시작하는 자동 생성 헤더를 제외한 실제 헤더 목록을 식별합니다.
        const allHeaders = new Set();
        jsonData.forEach(row => {
            Object.keys(row).forEach(header => allHeaders.add(header));
        });
        const headersToKeep = Array.from(allHeaders).filter(h => !h.startsWith('__EMPTY'));

        // 3. 유효한 헤더만 사용하여 데이터를 다시 구성합니다.
        ...
        // 4. 이제 완전히 비어버린 행을 최종적으로 필터링합니다.
        const finalData = cleanedData.filter(row =>
...
app.post('/headers', (req, res) => {
    ...
    try {
        ...
        // header: 1 옵션은 데이터를 2차원 배열로 만들고, 첫번째 요소가 헤더 배열입니다.
        const data = xlsx.utils.sheet_to_json(sheet, { header: 1 });
        const headers = data[0] || [];
        res.json(headers);
    } catch (error) {
        res.status(500).send('헤더를 가져오는 중 오류 발생');
    }
});
