        ...
        nonEmptyRows.forEach(row => {
            for (const header in row) {
                if (String(row[header]).trim() !== "") {
                    populatedHeaders.add(header);
                }
            }
        });

        if (populatedHeaders.size === 0) {
            return res.json([]);
        }

        // 4. 내용이 있는 열만 포함하여 최종 데이터 생성
        const finalData = nonEmptyRows.map(row => {
            const newRow = {};
            populatedHeaders.forEach(header => {
                // Ensure the header exists in the original row to avoid adding 'undefined'
