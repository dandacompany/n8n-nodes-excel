...
1,ProductA,100
2,ProductB,150`);
    const [createFileType, setCreateFileType] = useState('xlsx');
    ...
    const isCsvSelected = selectedFile && selectedFile.toLowerCase().endsWith('.csv');
    ...
    const handleCreate = async (e) => {
        e.preventDefault();
        if (!createFileName) return setError('File name is required.');

        const fullFileName = `${createFileName}.${createFileType}`;

        try {
            const payload = { fileName: fullFileName, csvData: createCsvData };
            ...
            <main>
                <section className="file-manager">
                    ...
                    <div className="file-controls">
                        ...
                        {sheets.length > 0 && !isCsvSelected && (
                            <select value={selectedSheet} onChange={e => setSelectedSheet(e.target.value)}>
                                {sheets.map(s => <option key={s} value={s}>{s}</option>)}
                            </select>
                        )}
                        {isCsvSelected && <span className="sheet-name-display">Sheet: {selectedSheet}</span>}
                        <input
                            type="number"
                            value={viewLimit}
                    ...
                    <form className="upload-form" onSubmit={handleUpload}>
                        <input type="file" onChange={e => setUploadFile(e.target.files[0])} accept=".xlsx, .csv" />
            ...
            <Modal show={showCreateModal} onClose={() => setShowCreateModal(false)}>
                <form onSubmit={handleCreate}>
                    ...
                    <div className="form-field">
                        ...
                        <input type="text" value={createFileName} onChange={e => setCreateFileName(e.target.value)} placeholder="e.g., my_data" required />
                    </div>
                    <div className="form-field">
                        <label>File Type:</label>
                        <div className="radio-group">
                            <label>
                                <input type="radio" value="xlsx" checked={createFileType === 'xlsx'} onChange={e => setCreateFileType(e.target.value)} />
                                Excel (.xlsx)
                            </label>
                            <label>
                                <input type="radio" value="csv" checked={createFileType === 'csv'} onChange={e => setCreateFileType(e.target.value)} />
                                CSV (.csv)
                            </label>
                        </div>
                    </div>
